...
type Tab = "planner" | "execution" | "reviewer";
...
function agentLabel(agent: string): string {
  if (agent === "memory") return "Reviewer";
...
export function WorkspaceApp() {
  ...
  useEffect(() => {
    if (workflow.phase === "rate_limit_pause") {
      setTab(workflow.rateLimitPausedAgent === "planner" ? "planner" : "execution");
  ...
  const inputPlaceholder = rateLimitInputEnabled
    ...
    : discourseInputEnabled
      ? workflow.humanInterventionActive
        ? "Guide the Executor and Reviewer…"
        : "Join the error discussion — guide the Executor and Reviewer…"
      : executingInterventionEnabled
        ? tab === "execution"
    ...
    <div className="app">
      <header className="header">
        ...
        {workflow.memoryAgentActive && !agentBusy && (
          <div className="agent-indicator reviewer-active">
            Reviewer — Error Resolution
            {workflow.errorResolutionAttempts > 0 &&
              ` (attempt ${workflow.errorResolutionAttempts})`}
          </div>
        )}
        {workflow.memoryCaptureActive && (
          <div className="agent-indicator reviewer-capture">Reviewer — Cataloging</div>
        )}
        {workflow.humanInterventionActive && (
        ...
        {workflow.phase === "rate_limit_pause" && workflow.rateLimitPausedAgent && (
          <div className="agent-indicator rate-limit-active">
            Paused — {agentLabel(workflow.rateLimitPausedAgent)} hit rate limit
          </div>
        )}
      </header>
      ...
      <nav className="tabs">
        ...
        >
          Planner
          <span className="tab-agent-num">Agent 1</span>
        </button>
        <button
        ...
        >
          Execution
          <span className="tab-agent-num">Agent 2</span>
        </button>
        <button
          className={tab === "reviewer" ? "active" : ""}
          onClick={() => setTab("reviewer")}
        >
          Reviewer
          <span className="tab-agent-num">Agent 3</span>
        </button>
      </nav>
          ...
          <section className="panel">
            <div className="panel-header">
              ...
              <p>
                ...
                Executor at any time. On errors, the Reviewer retries automatically.
              </p>
            </div>
        ...
        {tab === "reviewer" && (
          <section className="panel">
            <div className="panel-header">
              <h2>Reviewer</h2>
              <p>
                The Reviewer catalogs connections, tracks errors, and resolves failures with the
                Executor. Persisted to <code>.polaris/memory/</code> on your machine.
              </p>
            </div>
